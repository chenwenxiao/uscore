...
alloc_proc(void) {
     ...
     */
     //LAB5 2013011317 : (update LAB4 steps)
    /*
...
do_fork(uint32_t clone_flags, uintptr_t stack, struct trapframe *tf) {
    ...
    ret = -E_NO_MEM;
    //LAB4:EXERCISE2 2013011317
    //LAB8:EXERCISE2 2013011317 HINT:how to copy the fs in parent's proc_struct?
    /*
...
load_icode(int fd, int argc, char **kargv) {
    /* LAB8:EXERCISE2 2013011317  HINT:how to load the file with handler fd  in to process's memory? how to setup argc/argv?
     * MACROs or Functions:
    ...
    struct elfhdr tmp_elf;
    struct elfhdr *elf = &tmp_elf;
    ret = load_icode_read(fd, elf, sizeof(struct elfhdr), 0);
    if (ret) goto bad_elf_cleanup_pgdir;

    //(3.2) get the entry of the program section headers of the bianry program (ELF format)
    struct proghdr tmp_ph;
    struct proghdr *ph = &tmp_ph;
    ...
    //(3.4) find every program section headers
        ret = load_icode_read(fd, ph, sizeof(struct proghdr), elf->e_phoff + sizeof(struct proghdr) * phnum);
        if (ret) goto bad_cleanup_mmap;
    ...
    //(3.5) call mm_map fun to setup the new vma ( ph->p_va, ph->p_memsz)
     ...
     //(3.6.1) copy TEXT/DATA section of bianry program
        while (start < end) {
            ...
            ret = load_icode_read(fd, page2kva(page) + off, size, from);
            if (ret) goto bad_cleanup_mmap;
    ...
    int i;
    for (i = 0; i < argc; ++i)
        str_size += strnlen(kargv[i], EXEC_MAX_ARG_LEN) + 1;
    str_size = (str_size / sizeof(long) + 1) * sizeof(long);
    uintptr_t top = USTACKTOP - str_size;
    char **uargv = (char **)(top - argc * sizeof(char *));
    str_size = 0;
    for (i = 0; i < argc; ++i) {
        uargv[i] = top + str_size;
